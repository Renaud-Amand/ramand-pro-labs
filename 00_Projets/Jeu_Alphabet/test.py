...
while en_marche:
    ...
    if index < len(alphabet):
        ...
        if os.path.exists(chemin_image):
            ...
            rect_image = image_dessin.get_rect(centerx=screen_rect.centerx, centery=hauteur * 0.25)
            
            # La pastille blanche
            pygame.draw.circle(screen, BLANC, rect_image.center, 200) # Un peu plus grand (200)
            screen.blit(image_dessin, rect_image)

        # Dessin de la lettre
        image_lettre = ma_police.render(lettre, True, BLEU_ROI)
        rect_lettre = image_lettre.get_rect(centerx=screen_rect.centerx, centery=screen_rect.centery + 50)
        screen.blit(image_lettre, rect_lettre)

        # Dessin du mot
        image_objet = petite_police.render(objet, True, BLEU_ROI)
        rect_objet = image_objet.get_rect(centerx=screen_rect.centerx, bottom=screen_rect.bottom - 40)
        screen.blit(image_objet, rect_objet)

    else:
        # --- MODE RÉCOMPENSE ---
        texte_bravo = ma_police.render('Bravo !', True, (255, 0, 100))
        rect_bravo = texte_bravo.get_rect(center=(screen_rect.centerx, screen_rect.centery))
